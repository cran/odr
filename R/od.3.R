...
#'     and the proportion of level-3 clusters/groups assigned to treatment (\code{p}).
...
od.3 <- function(n = NULL, J = NULL, p = NULL, icc2 = NULL, icc3 = NULL, r12 = NULL, r22 = NULL,
  ...
  if (is.null(n)) {
    n.expr <- quote({
      sqrt((1 - icc2 - icc3) * (1 - r12)) /
        sqrt(icc3 * (1 - r32) * J + icc2 * (1 - r22)) *
        sqrt((1 - p) * (c3 + c2 * J) + p * (c3t + c2t * J)) /
        sqrt((1 - p) * c1 * J + p * c1t * J)
    })
  } else {
    n.expr <- quote(n)
  }
  ...
  if (nn[iter] - nn[iter-1] <= 10e-10 && JJ[iter] - JJ[iter-1] <= 10e-10 && pp[iter] - pp[iter-1] <= 10e-10) {
    nn <- JJ <- pp <- NULL
  } else {
    cat("===============================\n",
