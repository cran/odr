...
#' @param plots logical, provide variance plots if TRUE, otherwise not; default is TRUE.
...
#' @param Jlim the plot range for J, default is c(2, 50).
#' @param Klim the plot range for K, default is c(2, 50).
#' @param plim the plot range for p, default is c(0, 1).
#' @param varlim the plot range for variance, default is c(0, 0.05).
...
od.4 <- function(n = NULL, J = NULL, K = NULL, p = NULL, icc2 = NULL, icc3 = NULL, icc4 = NULL,
                 ...
                 vartitle = NULL,verbose = TRUE, iter = NULL) {
  funName <- "od.4"
  ...
  if (!is.null(iter))
    if (!is.numeric(iter) || iter < 2)
      stop("specified 'iter' must be numeric in [2, 10e10]")
  ...
  if (is.null(n)) {
    n.expr <- quote({
      sqrt((1 - icc2 - icc3 - icc4) * (1 - r12)) /
        ...
        sqrt((1 - p) * (c4 + c3 * K + c2 * J * K) +
               p * (c4t + c3t * K + c2t * J * K)) /
        sqrt((1 - p) * c1 * J * K + p * c1t * J * K)
    })
  } else {
    n.expr <- ({n})
  }
  if (is.null(J)) {
    J.expr <- quote({
      sqrt(n * icc2 * (1 - r22) + (1 - icc2 - icc3 - icc4) * (1 - r12)) /
        sqrt(n * K * icc4 * (1 - r42) + n * icc3 * (1 - r32)) *
        sqrt((1 - p) * (c4 + c3 * K) + p * (c4t + c3t * K)) /
        sqrt((1 - p) * (c2 * K + c1 * n * K) + p * (c2t * K + c1t * n * K))
    })
  } else {
    J.expr <- ({J})
  }
  if (is.null(K)) {
    K.expr <- quote({
      sqrt(n * J * icc3 * (1 - r32)  + n * icc2 * (1 - r22) +
        ...
        sqrt(n * J * icc4 * (1 - r42)) *
        sqrt((1 - p) * c4 + p * c4t) /
        sqrt((1 - p) * (c3 + c2 * J + c1 * n * J) + p * (c3t + c2t * J + c1t * n * J))
    })
  } else {
    K.expr <- ({K})
  }
  ...
  if (!is.null(J)) {
    if (!is.numeric(J) || J <= 0)
      stop("constrained 'J' must be nu meric in (0, 10e10)")
  } else {
    J <- sample(2:50, 1)
  }
  if (!is.null(K)) {
    if (!is.numeric(K) || J <= 0)
      stop("constrained 'K' must be nu meric in (0, 10e10)")
  } else {
    K <- sample(2:50, 1)
  }
  if (!is.null(p)) {
  ...
  if (nn[iter] - nn[iter-1] <= 10e-10 && JJ[iter] - JJ[iter-1] <= 10e-10 &&
      KK[iter] - KK[iter-1] <= 10e-10 && pp[iter] - pp[iter-1] <= 10e-10) {
    nn <- JJ <- pp <- KK <- NULL
  } else {
    cat("===============================\n",
  ...
  var.expr <- quote({
    ...
    (icc4 * (1 - r42) + icc3 * (1 - r32) / K + icc2 * (1 - r22) / (J * K) +
        (1 - icc2 - icc3) * (1 - r12) / (n * J * K)) / (p * (1 - p) * L)
  })
  Var <- eval(var.expr)
  par <- c(par, list(m = m))
