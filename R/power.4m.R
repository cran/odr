...
power.4m <- function(cost.model = TRUE, expr = NULL, constraint = NULL,
  ...
  if (!is.null(expr)) {
    if (expr$funName != "od.4m") {
      stop("'expr' can only be NULL or
           the return from the function of 'od.4m'")
    } else {
      if (sum(sapply(list(icc2, icc3, icc4, r12, r22, r32, r42m, c1, c2, c3, c4,
                          c1t, c2t, c3t, omega, n, J, K, p), function(x) {!is.null(x)})) >= 1)
        stop("parameters of 'icc2', 'icc3', 'icc4', 'r12', 'r22', 'r32', 'r42m',
             'c1', 'c2', 'c3', 'c4', 'c1t', 'c2t', 'c3t', 'omega', 'n', 'J', 'K', and 'p'
             have been specified in expr of 'od.4m'")
      icc2 <- expr$par$icc2
      icc3 <- expr$par$icc3
      icc4 <- expr$par$icc4
      r12 <- expr$par$r12
  ...
  if (sum(sapply(list(r12, r22, r32, r42m), function(x) {
    NumberCheck(x) || any(0 > x | x >= 1)
  })) >= 1) stop("'r12', 'r22', 'r32', and 'r42m' must be numeric in [0, 1)")
  if (cost.model == TRUE){
    if (sum(sapply(list(c1, c2, c3, c4, c1t, c2t, c3t), function(x) {
      NumberCheck(x) || x < 0})) >= 1)
      stop("'c1', 'c2', 'c3', 'c4', 'c1t', 'c2t', 'c3t' must be numeric in [0, Inf)")
    if (NumberCheck(m))
  ...
  if(cost.model == TRUE) {
    ...
    if (is.null(power)) {
      ...
      out <- c(out, list(L = out$m / ((1 - p) * (c1 * n * J * K + c2 * J * K + c3 * K) +
                                        p * (c1t * n * J * K + c2t * J * K + c3t * K) + c4)))
    } else if (is.null(d)) {
